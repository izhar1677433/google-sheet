...
function DropdownOwnedBy() {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        className={`flex items-center px-3 py-1 rounded  text-[14px] font-normal  focus:outline-none `}
        onClick={() => setOpen((v) => !v)}
      >
        {selected}
        <svg
      ...
      {open && (
        <div className="absolute left-0 mt-2 w-48 bg-white rounded shadow-lg border border-gray-100 z-50 animate-fade-in">
          {options.map((opt) => (
            <div
              key={opt}
              className={`px-4 py-2 cursor-pointer text-gray-900 text-[15px] hover:bg-gray-100 flex items-center ${selected === opt ? "font-semibold text-black" : ""}`}
              onClick={() => {
                setSelected(opt);
                setOpen(false);
              }}
